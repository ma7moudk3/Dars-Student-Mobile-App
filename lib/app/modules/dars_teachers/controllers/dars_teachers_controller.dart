...
import '../data/repos/dars_teachers_repo.dart';
import '../data/repos/dars_teachers_repo_implement.dart';

class DarsTeachersController extends GetxController {
  ...
  PagingController<int, DarsTeacher> pagingController =
      PagingController(firstPageKey: 1); // item = dars teacher
  ...
  final DarsTeachersRepo _darsTeacherRepo = DarsTeachersRepoImplement();
  ...
  Future _getCountries() async {
    countries = await _darsTeacherRepo.getCountries();
  ...
  Future _getClasses() async {
    classes = await _darsTeacherRepo.getClasses();
  ...
  Future _getTopics() async {
    topics = await _darsTeacherRepo.getTopics();
  ...
  Future _getSkills() async {
    skills = await _darsTeacherRepo.getSkills();
  ...
  void _initPageRequestListener() {
    pagingController.addPageRequestListener((int pageKey) async {
      await getDarsTeachers(page: pageKey);
    });
  }

  Future getDarsTeachers({
    required int page,
  }) async {
    try {
      if (await checkInternetConnection(timeout: 10)) {
        darsTeachers = await _darsTeacherRepo.getDarsTeachers(
          page: page,
          perPage: _pageSize,
        ...
        if (isLastPage) {
          pagingController.appendLastPage(darsTeachers);
        } else {
          final nextPageKey = page + 1;
          pagingController.appendPage(darsTeachers, nextPageKey);
        }
      } else {
